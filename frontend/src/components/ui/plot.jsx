...
function temValores(dados, dim) {
    if (!dim)
        return false; // não existe
    return dados.some((d) => d.dimensoes[dim] !== undefined && d.dimensoes[dim] !== null);
}
export default function PlotGeneric({ tipo, dados, tamanho = null }) {
    const [brasil, setBrasil] = useState(null);
    ...
    const fillDim = temValores(dados, terceiraDim) ? terceiraDim :
    ...
    const renderPlot = () => {
        switch (tipo) {
            ...
            case "graficos_linhas":
                ...
                const dadosOrdenados = [...dados].sort((a, b) => a.dimensoes[chaveNumerica] - b.dimensoes[chaveNumerica]);
                // 2. Extrai apenas os valores formatados para o Plot
                const dadosGraficoLinhasFormatados = dadosOrdenados.map((d) => ({
                        ...
                        marks: [
                            Plot.geo(brasil, {
                                fill: (feature) => {
                                    ...
                                    return dados.find((d) => Object.values(d.dimensoes)[0] === estado)?.valor;
                                },
                                stroke: "lightgray",
                            }),
                        ],
