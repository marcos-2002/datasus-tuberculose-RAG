...
import { Settings, Send, Users, Loader2, } from "lucide-react";
...
Clique no botão "Ver informações disponíveis" para ver quais informações eu tenho disponível para responder.`,
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            setJsonPlot(JSON.stringify({
                visualizacoes: JSON.parse(response.data?.json_plot).visualizacoes?.map((visualizacao) => {
            ...
            <div className="flex-1 flex flex-col mt-14 md:ml-64 overflow-hidden">
                ...
                <Dialog open={isDataModalOpen} onOpenChange={setIsDataModalOpen}>
                    <DialogContent className="max-w-md">
                        ...
                        <ul className="list-disc list-inside max-h-80 overflow-auto text-sm text-gray-700">
                            {availableDataList.map((item, idx) => (<li key={idx}>{item}</li>))}
                        </ul>
                    </DialogContent>
                </Dialog>
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    <div className="space-y-4 max-w-4xl">
                        {messages.map((msg, idx) => (<div key={idx} className={`flex items-start space-x-3 ${msg.position === "R" ? "justify-end" : ""}`}>
                                {msg.position === "L" && (<div className="w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                                ...
                                {msg.position === "R" && (<div className="w-7 h-7 bg-gray-200 rounded-full flex items-center justify-center flex-shrink-0">
                                        <Users className="w-3.5 h-3.5 text-gray-500"/>
                                    </div>)}
                                {msg.sql != null && msg.position === "L" && (<Dialog>
                                        <DialogTrigger asChild>
                                            <Button variant="ghost" className="text-xs mt-2 text-blue-600 hover:underline p-0 h-auto" onClick={() => setSelectedSQL(msg.sql)}>
                                                Detalhes
                                            </Button>
                                        </DialogTrigger>
                ...
                <div className="bg-white border-t border-gray-100 p-5 shrink-0">
                    <div className="flex items-center space-x-2 mb-3">
                        <Input placeholder="Digite sua pergunta ou comando..." className="flex-1 text-sm" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter")
                sendMessage();
        }} disabled={loading}/>
